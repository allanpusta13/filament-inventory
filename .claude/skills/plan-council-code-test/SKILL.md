...
description: Use for any non-trivial coding task in this project — a plan is written first, reviewed by a multi-perspective council before any code is written, then implemented and verified against the FULL test suite (not just new tests) plus a Playwright visual check and Impeccable design audit for UI-touching work, then cleaned up and committed, before moving to the next task. Triggers on requests to implement, build, add, fix, or change functionality in the codebase.
---
...
Run tests using **Pest** for backend/PHP work, and **Vitest + React Testing Library** for isolated frontend component logic. This project uses **Pest v4's built-in browser testing** for UI-touching changes — never Laravel Dusk, and never `$this->browse()` syntax. Use `visit()` directly. `wait()` takes seconds, not milliseconds. Pest browser tests render real HTML through Inertia, so they work regardless of React underneath — they're testing the actual user-facing flow.

- `php artisan test` — run the **entire** backend suite, not just tests related to this task.
- `npm run test` — run the **entire** Vitest suite if the task touched any React component.
- If the change touches a user-facing flow, also run the relevant Pest v4 browser tests (`tests/Browser/`).
- Don't write Vitest tests for shadcn's generated primitives themselves (`components/ui/`) — test the app code that uses them, not the library code.

**Design check (UI-touching tasks only):** functional correctness (Pest/Vitest passing) is necessary but not sufficient for a UI task — it doesn't catch generic, templated-looking design, and it doesn't catch a page that's functionally wired correctly but visually broken (e.g. rendering unstyled). For any task that adds or changes a page/component's visual design, once the Pest browser test confirms it renders and functions correctly:

1. **Use the Playwright browser skill** to actually navigate to the affected page and take a screenshot — confirm what's rendering matches what was intended, rather than inferring it from the code you just wrote. This is especially important for anything involving shadcn components or styling, where "the code looks right" and "it actually renders right" have turned out to diverge before in this project.
...
**Both the pre-existing tests and any new tests written for this task must pass, across both suites where applicable.** A green result on new tests alone is not sufficient — this task is only done when everything relevant is green, confirming nothing else regressed.

- If **any** test fails (old or new, either suite), or the Playwright screenshot doesn't match intent, or Impeccable flags unresolved design issues → go to Step 5.
...
Diagnose the root cause of the failure (don't just patch the symptom) and fix it. For design/rendering issues flagged by Playwright or Impeccable specifically, prefer composition-level fixes (Tailwind classes, layout structure, which shadcn primitives you combine and how) over hand-editing shadcn's generated primitives in `components/ui/` — those stay untouched per CLAUDE.md. If a Playwright screenshot revealed something isn't rendering as coded (not just a style preference), diagnose why before assuming a styling tweak will fix it — that gap is often a wiring problem, not a CSS problem. Return to Step 4 and re-run everything again — the full test suite(s) and, for UI tasks, the Playwright screenshot + Impeccable check — not just the previously failing item, since a fix can introduce a new regression elsewhere. Repeat until everything is green and clean across Pest, Vitest, Playwright, and Impeccable.
...
- **Cleanup**: remove any scratch files, debug output, commented-out old code, and unused imports introduced while working the task. Run `vendor/bin/pint --format agent` on changed PHP files and `npm run lint -- --fix` on changed TS/JS files. Re-run the full suite(s) once more after cleanup — a cleanup pass can break something too, so it isn't exempt from Step 4's rule.
...
- "All tests" in Step 4 means the whole project's Pest suite, every time, not an incremental subset — this is what catches regressions early instead of at the end of a long session.
