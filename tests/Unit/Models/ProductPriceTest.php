...
it('casts cost_price and sale_price as decimal strings', function () {
    $price = ProductPrice::factory()->create(['cost_price' => 12.3456, 'sale_price' => 99.99]);
...
it('rejects a second is_current row for the same variant at the DB level', function () {
    $variant = ProductVariant::factory()->create();
    ProductPrice::factory()->for($variant, 'variant')->create(['is_current' => true]);

    expect(fn () => ProductPrice::factory()->for($variant, 'variant')->create(['is_current' => true]))
...
it('allows multiple non-current rows for the same variant', function () {
    ...
    ProductPrice::factory()->notCurrent()->for($variant, 'variant')->create();
    ProductPrice::factory()->notCurrent()->for($variant, 'variant')->create();
    $current = ProductPrice::factory()->for($variant, 'variant')->create();
...
it('flips the previous current row to false when recording a new price', function () {
    $variant = ProductVariant::factory()->create();
    $old = ProductPrice::factory()->for($variant, 'variant')->create(['sale_price' => 100]);

    $new = ProductPrice::recordNewPrice($variant, costPrice: 50, salePrice: 120);
